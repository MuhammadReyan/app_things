    ...
    <div class="side-nav pl-sm-0 pl-lg-4 pl-xl-4">
        <ul class="list-unstyled modules">
            <li class="module" v-for="(menu, menu_key_id) in menu_sorted" v-bind:key="menu.sort_order" :class="{ '': menu.menu_key == menu_key}">
                ...
                <a :href="(typeof menu.sub_menu == 'undefined' || menu.sub_menu == '')?menu.route:'#menu'+menu_key_id" :data-toggle="(typeof menu.sub_menu != 'undefined')?'collapse':''" aria-expanded="false" :class="{ 'dropdown-toggle': typeof menu.sub_menu != 'undefined', 'selected-parent-menu': menu.menu_key == menu_key }" class="module-parent">
