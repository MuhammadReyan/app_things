        ...
        <div class="col-md-12">
            ...
            <form @submit.prevent="submit_form" class="mb-3">
                ...
                <div class="d-flex flex-wrap mb-4">
                    <div class="mr-auto">
                        <span class="text-title" v-if="invoice_slack == ''">{{ $t("Add Invoice") }}</span>
                        <span class="text-title" v-else>{{ $t("Edit Invoice") }}</span>
                    </div>
                    <div class="">
    ...
    export default {
        ...
        data(){
            return{
                ...
                bill_to : (this.invoice_data == null)?'':this.invoice_data.bill_to,
                bill_to_slack : (this.invoice_data == null)?'':(this.invoice_data.bill_to == 'SUPPLIER')?this.invoice_data.supplier.slack:this.invoice_data.customer.slack,
                bill_to_label : (this.invoice_data == null)?'':this.invoice_data.bill_to_name,

                invoice_slack : (this.invoice_data == null)?'':this.invoice_data.slack,
                invoice_reference : (this.invoice_data == null)?'':(this.invoice_data.invoice_reference != null)?this.invoice_data.invoice_reference:'',
                ...
                terms : (this.invoice_data == null)?'':(this.invoice_data.terms != null)?this.invoice_data.terms:'',
                grand_total : 0,
                ...
                invoice_product_list : (this.invoice_data != null)?this.invoice_data.products:[],
        ...
        mounted() {
            console.log('Add invoice page loaded');
            this.$refs.bill_to_label.setSearchData(this.bill_to_label);
        },

        created() {
            this.update_product_list(this.invoice_product_list);
        },
        ...
        methods: {
            ...
            update_product_list(invoice_product_products) {
                if(invoice_product_products != null && invoice_product_products.length > 0){
                    this.products = [];
                    for (let i = 0; i < invoice_product_products.length; i++) {
                        var individual_product = {
                            slack: invoice_product_products[i].product_slack,
                            name : invoice_product_products[i].name,
                            quantity : invoice_product_products[i].quantity,
                            unit_price : invoice_product_products[i].amount_excluding_tax,
                            discount_percentage : invoice_product_products[i].discount_percentage,
                            tax_type : (typeof invoice_product_products[i].tax_type != 'undefined' && invoice_product_products[i].tax_type != '')?invoice_product_products[i].tax_type:'EXCLUSIVE',
                            tax_percentage : invoice_product_products[i].tax_percentage,
                            amount : invoice_product_products[i].total_amount
                        };
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        ...
                        this.$on("submit",function () {
                            ...
                            formData.append("invoice_reference", (this.invoice_reference != null)?this.invoice_reference:'');
                            formData.append("invoice_date", this.convert_date_format(this.invoice_date));
                            formData.append("invoice_due_date", this.convert_date_format(this.invoice_due_date));
