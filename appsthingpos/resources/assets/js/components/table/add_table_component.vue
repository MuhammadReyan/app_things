            ...
            <form @submit.prevent="submit_form" class="mb-3">
                ...
                <div class="form-row mb-2">
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="table_number" v-model="table_number" v-validate="'required|max:250'" class="form-control form-control-custom" :placeholder="$t('Please enter table name')"  autocomplete="off">
                        <span v-bind:class="{ 'error' : errors.has('table_number') }">{{ errors.first('table_number') }}</span> 
                    </div>
                    <div class="form-group col-md-3">
                        <label for="no_of_occupants">{{ $t("No. of Occupants") }}</label>
                        <input type="number" name='no_of_occupants' v-model="no_of_occupants" v-validate="'required|numeric'" class="form-control form-control-custom" :placeholder="$t('Please enter no of occupants')"  autocomplete="off" step="1" min="0">
                        <span v-bind:class="{ 'error' : errors.has('no_of_occupants') }">{{ errors.first('no_of_occupants') }}</span> 
                    </div>
                    <div class="form-group col-md-3">
                ...
                <div class="form-row mb-2">
                    <div class="form-group col-md-3">
                        ...
                        <select name="waiter" v-model="waiter" v-validate="''" class="form-control form-control-custom custom-select">
                            <option value="">Choose Default Waiter..</option>
                            <option v-for="(waiter_item, index) in waiter_list" v-bind:value="waiter_item.slack" v-bind:key="index">
