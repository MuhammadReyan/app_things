            ...
            <form @submit.prevent="submit_form" class="mb-3">
                ...
                <div class="form-row mb-2">
                    <div class="form-group col-md-3">
                        ...
                        <input type="text" name="category_name" v-model="category_name" v-validate="'required|max:250'" class="form-control form-control-custom" :placeholder="$t('Please enter category name')"  autocomplete="off">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select name="display_on_pos_screen" v-model="display_on_pos_screen" v-validate="'required|numeric'" class="form-control form-control-custom custom-select">
                            <option value="">Choose Show on POS Screen..</option>
                            <option v-for="(display_on_pos_screen_option, index) in display_on_pos_screen_options" v-bind:value="index" v-bind:key="index">
                                {{ display_on_pos_screen_option }}
                            </option>
                        </select>
                        <span v-bind:class="{ 'error' : errors.has('display_on_pos_screen') }">{{ errors.first('display_on_pos_screen') }}</span>  
                    </div>
                    <div class="form-group col-md-3">
                        <label for="display_on_qr_menu">{{ $t("Show on QR Menu") }}</label>
                        <select name="display_on_qr_menu" v-model="display_on_qr_menu" v-validate="'required|numeric'" class="form-control form-control-custom custom-select">
                            <option value="">Choose Show on QR Menu..</option>
                            <option v-for="(display_on_qr_menu_option, index) in display_on_qr_menu_options" v-bind:value="index" v-bind:key="index">
